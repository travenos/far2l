...
inline wchar_t CellBaseChar(const CHAR_INFO &ci)
{
...
inline Class Classify(const CHAR_INFO &ci)
{
...
inline bool ReorderLine(CHAR_INFO *line, unsigned int cw, unsigned int *vis2log = nullptr)
{
	if (vis2log) {
...
inline bool RowHasRTL(unsigned int cy)
{
...
inline void ExpandDirtyArea(SMALL_RECT &area)
{
	if (!g_winport_con_out)
		return;
...
inline unsigned int VisualColumnToLogical(unsigned int cy, unsigned int vis_x)
{
	if (!g_winport_con_out)
		return vis_x;
...
inline unsigned int LogicalColumnToVisual(unsigned int cy, unsigned int log_x, unsigned int cw)
{
